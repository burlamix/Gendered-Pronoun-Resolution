...
def get_inputs_and_label ( sent ):
    inputs = np.hstack ( (sent.embeddings[ sent.pron_tok_off ], sent.embeddings[ sent.A_tok_off ], sent.embeddings[ sent.B_tok_off ] ) )
...
def load_embeddings_minibatch ( dataset, size=0, shuffle=False ):
    ...
    for i in count_iterator:
        sent = dataset[i] if not shuffle else dataset[ random.randint (0, len(dataset)-1) ]
        sent_inputs, sent_label = get_inputs_and_label (sent)
        minibatch_inputs_list.append ( sent_inputs )
        minibatch_labels_list.append ( sent_label )
        minibatch_ids_list.append (sent.id)
...
def compute_predictions ( train_fname, validation_fname, test_fname, output_fname ):
    ...
    weights = [0,0,0]
    train_dataset = []
    for sent in parse_embeddings_dataset(train_fname):
        train_dataset.append (sent)
        label = 0 if sent.A_coref else 1 if sent.B_coref else 2 
        weights[label] += 1
    ...
    validation_dataset = list ( parse_embeddings_dataset (validation_fname ))
    logger.info ("loaded  validation dataset: {} sentences".format(len(validation_dataset)))
    
    # input: 3 embeddings (pronoun, A, B) 
    net = BaselineNet ( 3*embeddings_size )
    ...
    ids, inputs, _ = load_embeddings_minibatch ( list (parse_embeddings_dataset ( test_fname )) )
